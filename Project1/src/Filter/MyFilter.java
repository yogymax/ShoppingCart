...
public class MyFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(uri.endsWith("record/register")){
			request1.getRequestDispatcher("/WEB-INF/jsp/register.jsp").forward(request1, response1);
		}
		else if(uri.endsWith("role/add")){
			request1.getRequestDispatcher("/WEB-INF/jsp/role.jsp").forward(request1, response1);
		}
        // (If session is not set)
		else if( (session==null || session.getAttribute("users")==null)&& (!uri.endsWith("login.jsp") && !uri.endsWith("record/authenticate")&& !uri.endsWith("record/add")&& !uri.endsWith("role/save"))){
